...
class WPIMItem extends WPIMDB {
	...
	public function get_all( $args = NULL, $get_counts = FALSE, $bypass_parse_order = FALSE ) {
			...
			foreach ( $potential_args AS $arg_key => $arg_label ) {
				...
				if ( ! empty( $args[ $arg_key ] ) || ! empty( $args[ $arg_label_key ] ) ) {
					...
					if ( is_array( $value ) ) {
						$in    = array_fill( 0, count( $value ), '%s' );
						$in    = implode( ',', $in );
						$where = $this->append_where( $where, $this->wpdb->prepare( " i.{$arg_key} IN ({$in})", $value ) );
					} else {
						$where = $this->append_where( $where, $this->wpdb->prepare( " i.{$arg_key} = %s", $value ) );
					}
				}
			}
		...
		if ( $get_counts ) {
			$query = 'SELECT count(DISTINCT i.inventory_id) FROM ' . $this->inventory_table . ' AS i ' . $from . $where;
			$query = apply_filters( 'wpim_item_count_query', $query, 'SELECT count(*) FROM ' . $this->inventory_table . ' AS i ', $from, $where, $args );
