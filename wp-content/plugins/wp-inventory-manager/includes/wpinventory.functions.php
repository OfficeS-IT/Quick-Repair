...
function wpinventory_reserve_form( $args = NULL ) {
	...
	if ( isset( $_POST['wpinventory_reserve_submit'] ) || isset( $_POST['_wpim_reserve_submit'] ) ) {
		...
		if ( ! isset( $_POST['_wpim_reserve_nonce'] ) || ! wp_verify_nonce( $_POST['_wpim_reserve_nonce'], 'WPInventoryReserveNonceAction' ) ) {
			$error = WPIMCore::__( 'Security error.  Please try again.' ) . '<br>';
		}
...
//		 Ensure value is one here so can properly validate....
		...
		$error                = apply_filters( 'wpim_reserve_form_errors', $error, $data );

		if ( ! $error ) {
			$success = wpinventory_process_reserve( $data );
			if ( $success === TRUE ) {
				$display = FALSE;
				$message = apply_filters( 'wpim_reserve_confirmation_message', $args['reserve_message'] );
			} else {
				$error = $success;
			}
		}
...
function wpinventory_process_reserve( $data ) {
	...
	*/
	...
	$response = '';
	$success  = wp_mail( $to_email, mb_convert_encoding( $subject, 'UTF-8', 'HTML-ENTITIES' ), mb_convert_encoding( $message, 'UTF-8', 'HTML-ENTITIES' ) );
	...
	if ( $send_confirmation ) {
		...
		$success = wp_mail( $confirm_email, mb_convert_encoding( $subject, 'UTF-8', 'HTML-ENTITIES' ), mb_convert_encoding( $message, 'UTF-8', 'HTML-ENTITIES' ) );
		if ( ! $success ) {
